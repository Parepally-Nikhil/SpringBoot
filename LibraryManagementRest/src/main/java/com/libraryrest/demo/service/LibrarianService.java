...
import com.libraryrest.demo.dao.StudentDao;
import com.libraryrest.demo.exception.InvalidDataException;
import com.libraryrest.demo.exception.LibrarianNotFoundException;
...
public class LibrarianService implements LibrarianServiceInt{
	...
	public boolean librarianLogin(Librarian librarian) {
		...
		throw new LibrarianNotFoundException("406 ","Wrong librarian credentials! try again");
	...
	public int issueBook(HttpServletRequest request,String bookId,String studentId,IssueBook ib) {
		...
		HttpSession session=request.getSession();
		String libid=(String) session.getAttribute("librarian");
		if(!(ib.getIssuedDate().matches("[0-9]{4}-[0-1][0-9]-[0-3][0-9]"))) {
			available=5;
			return available;
		}
		DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
	...
	public IssueBook checkFine(String bid,String sid,String rdate) {
		...
		for(IssueBook ib:ibl) {
			if(ib.getBook().getBookId().equals(bid) && ib.getStudent().getStudentId().equals(sid)) {
				...
				if(!(rdate.matches("[0-9]{4}-[0-1][0-9]-[0-3][0-9]"))) {
					throw new InvalidDataException("406 ","Please follow the requested date format yyyy-mm-dd");
				}
				if(fine!=0) {
					fine=0.0;
				}
		...
		throw new InvalidDataException("406 ","Please enter valid credentials");
	}
}
